...
>
> All 13 available documents (of the 14 originally listed — see Tier 3 note) are downloaded, verified to contain real extractable text (not scanned images — checked file-by-file, one initial IPC download had to be redone after this check caught a scanned/image-only version), and ingested into the vector store (5,091 chunks). **Checked below reflects actual downloaded state, not a plan.**
