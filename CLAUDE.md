...
- **Encryption at rest — DESIGN (2026-08-24), not implemented**: target design, should this move beyond portfolio scope — SQLite via `SQLCipher` (drop-in encrypted-at-rest replacement, minimal code change: swap the driver, add a key parameter to the connection string) for case data; evidence files (once file upload exists) encrypted individually with `Fernet` (symmetric, from Python's `cryptography` package) before writing to disk, key held server-side only, never in the DB alongside the data it encrypts. ChromaDB's own SQLite-backed persistence would need the same SQLCipher treatment or a self-managed encrypted volume, since Chroma has no native at-rest encryption option. Key management itself (env var vs. secrets manager vs. KMS) is deliberately left unspecified — that decision depends on the actual deployment target, which doesn't exist yet (§19: local/dev only). Not implemented now: SQLCipher requires a different Python driver (`pysqlcipher3` or similar) and connection-string changes across `db.py`/`config.py`, which is real work disproportionate to a local-only dev build with no real user data.
...
- **Data retention — DESIGN (2026-08-24), not implemented**: target design — a `DELETE /cases/{id}` endpoint that hard-deletes the case row and all rows in every relationship's cascade (`Case`'s existing `cascade="all, delete-orphan"` relationships already make this a one-line delete at the ORM level, no manual cleanup needed across 8 related tables), satisfying DPDP's erasure-on-request expectation. No automatic time-based expiry designed — session-only/no-auth (§18) means there's no natural "session ended" signal to trigger one off of; a cron-style sweep would need its own policy (delete after N days of no `AuditLogEntry` activity is the most defensible signal, mirroring "purpose fulfilled") that hasn't been decided. Not implemented now: the endpoint itself is a small addition, but doing it properly (soft-delete-then-purge vs. hard delete, whether audit-log entries for a deleted case must also disappear or should outlive the case for compliance purposes — these two goals conflict) needs a decision this session didn't reach, not just an afternoon of coding.
